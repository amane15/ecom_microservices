	...
	if err != nil {
		...
		case errors.As(err, &unmarshTypeError):
			if unmarshTypeError.Field != "" {
				return fmt.Errorf("body contains incorrect JSON type of field %q", unmarshTypeError.Field)
			}
...
func (app *application) readInt(qp url.Values, param string, defaultValue int) int {
	s := qp.Get(param)
	if s == "" {
		return defaultValue
	}
