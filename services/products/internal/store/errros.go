...
func handlePostgresErrors(err error, pgError *pgconn.PgError) error {
	switch pgError.Code {
	case "23505":
		return wrapError("unique violation", pgError, data.ErrDuplicateSlug)
	case "23503":
		return wrapError("foreign key violation", pgError, data.ErrForeignKeyViolation)
	case "23502":
		return wrapError("not null violation", pgError, data.ErrNotNullViolation)
	case "23514":
		return wrapError("check violation", pgError, data.ErrCheckViolation)
	default:
